...
function atw_slider_shortcode( $args = '' ) {
    ...
    $tail = "</div></div><div class='atw-clear-both'></div><!-- atwslider -->\n";

    $slider_post_slug = atw_posts_get_slider_opt( 'slider_post_slug', $name);

	// generate actual images here....
    ...
    if (atw_posts_get_slider_opt('pager', $name) == 'sliding' && !empty($GLOBALS['atw_slider_thumbs'])) {
        ...
        foreach ($GLOBALS['atw_slider_thumbs'] as $thumb ) {
            $sliding .= '<div class="atwk-slide"><img src="' .  apply_filters('atw_slider_no_http',$thumb) . '" /></div>' . "\n";
        }
        $sliding .= "</div></div><!-- $id -->\n";
    }
...
function atw_slider_emit_css( $name ) {
	...
	if ( atw_posts_get_slider_opt ( 'directionVertical', $name) != '' )
		$margin = '0';	// margins don't work for vertical slider
    ...
    if ( atw_posts_get_slider_opt( 'sliderWidth', $name)) {    // make it not take full width
        ...
        switch ( $position ) {
            ...
            case 'center':
                ...
                break;
            case 'floatLeft':
                $css .= 'float:left;margin-right:2%;';
                break;
            case 'floatRight':
                $css .= 'float:right;margin-left:2%;';
                break;
            default;
                break;
        }
    ...
    if (atw_posts_get_slider_opt( 'pager', $name ) == 'none') {
        $css .= $my_class . ' .atwkslider .atwk-control-nav{margin-bottom:0em !important;}';
		$css .= $my_class . ' .atwkslider .atwk-direction-nav{margin-bottom:0em !important;}';
		$css .= 'body .atwkslider .atwk-direction-nav{margin-bottom:0em !important;}';
		$css .= 'body .atwkslider .atwk-control-nav{margin-bottom:0em !important;}';
		$css .= '.atwkslider{margin-bottom:0 !important;}';
    }
...
function atw_slider_do_footer() {
    ...
    for ($i = 1 ; $i <= $sliders_count ; $i++) {
		...
		$val = atw_posts_get_slider_opt( 'pausePlay', $name );		// disable pauseOnHover if showing pause/play control
		if ( $val != '' ) {
